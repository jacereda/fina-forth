...
#define POPDC                                                                  \
        dc = (((DCELL)tos) << CELLSHIFT) | *(UCELL *)dsp++;                    \
        tos = *dsp++
#define POPDC2                                                                 \
        dc2 = (((DCELL)tos) << CELLSHIFT) | *(UCELL *)dsp++;                   \
        tos = *dsp++
#define PUSHDC                                                                 \
        *--dsp = tos;                                                          \
        *--dsp = dc;                                                           \
...
#undef DEBUG
#if defined DEBUG
__attribute__((noinline)) static void loc(const char *p) {
        char c;
        while ((c = *p++))
                Sys_PutChar(c);
        Sys_PutChar('\n');
...
static inline UCELL UMSlashMod(UCELL *dd, UCELL d, UCELL *pmod) {
        UCELL s = CELLSHIFT - 1;
        UCELL i = CELLSHIFT;
        UCELL h = dd[0];
        UCELL l = dd[1];
